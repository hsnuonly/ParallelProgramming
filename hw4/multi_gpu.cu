...
void block_FW(int B, char* outFileName) {
    ...
    for(int i=0;i<2;i++){
        ...
        cudaMallocPitch(&device_d[i],&pitch[i],sizeof(int)*round*B,round*B);
        cudaMemcpy2DAsync(device_d[i],pitch[i],d,sizeof(int)*n,sizeof(int)*n,n,cudaMemcpyHostToDevice);
    }
	...
	for (int r = 0; r < round; ++r) {
        ...
        cudaSetDevice(dev);
        dimGrid = dim3(1,1);
        kernel_I  <<<dimGrid,dimBlock,0>>>(device_d[dev],pitch[dev],r,r,n,B,r);
        dimGrid = dim3(1,round);
        kernel_III<<<dimGrid,dimBlock,0>>>(device_d[dev],pitch[dev],r,0,n,B,r);
        cudaDeviceSynchronize();
        auto mem_beg = std::chrono::high_resolution_clock::now();    
        cudaMemcpy2D(device_d[!dev]+r*B*pitch[!dev],pitch[!dev],
                    device_d[dev]+r*B*pitch[dev],pitch[dev],
                    sizeof(int)*n,B,cudaMemcpyDefault);
        auto mem_end = std::chrono::high_resolution_clock::now();
        k_sync_time+=std::chrono::duration<double>(mem_end-mem_beg).count();

        
        cudaSetDevice(0);
        dimGrid = dim3((round>>1),1);
        kernel_III<<<dimGrid,dimBlock,0>>>(device_d[0],pitch[0],0,r,n,B,r);
        cudaSetDevice(1);
        dimGrid = dim3(round-(round>>1),1);
        kernel_III<<<dimGrid,dimBlock,0>>>(device_d[1],pitch[1],(round>>1),r,n,B,r);

        cudaSetDevice(0);
        dimGrid = dim3((round>>1),round);
        kernel_III<<<dimGrid,dimBlock,0>>>(device_d[0],pitch[0],0,0,n,B,r);
        cudaSetDevice(1);
        dimGrid = dim3(round-(round>>1),round);
        kernel_III<<<dimGrid,dimBlock,0>>>(device_d[1],pitch[1],(round>>1),0,n,B,r);
    ...
    comp_time += std::chrono::duration<double>(comp_end-comp_beg).count()-k_sync_time;
    mem_time += k_sync_time;
    mem_beg = std::chrono::high_resolution_clock::now();
    cudaMemcpy2D(d,sizeof(int)*n,
        device_d[0],pitch[0],
        sizeof(int)*n,min(B*(round>>1),n),cudaMemcpyDeviceToHost);
    cudaMemcpy2D(d+n*(round>>1)*B,sizeof(int)*n,
        device_d[1]+pitch[1]*(round>>1)*B,pitch[1], 
        sizeof(int)*n,n-B*(round>>1),cudaMemcpyDeviceToHost);
