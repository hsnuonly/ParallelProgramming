...
int my_merge(float *a, float *b, int m, int n, int d)
{
    int i = 0, j = 0;
    int count;
    if (d == 0)
...
void half_swap(MPI_Request *req, float *a, float *b, int m, int n, int d)
{
    MPI_Status status;
    MPI_Wait(req, &status);
    ...
    */
    ...
    for (int j = 0;; j++)
    {
        ...
        else
        {
            ...
            if (rank % 2 == j % 2 && rank + 1 < size && buf[slice_proc - 1] > rightmost)
            {
                t = std::thread(&half_swap, req, buf + slice_proc - n, right, n, n, 1);
            }
            if (rank % 2 != j % 2 && rank > 0 && buf[0] < leftmost)
            {
                MPI_Wait(&req[2], &status);
                MPI_Wait(&req[3], &status);
                my_merge(left, buf, n, n, 2);
            }
            if (rank % 2 == j % 2 && rank + 1 < size && buf[slice_proc - 1] > rightmost)
            {
                t.join();
