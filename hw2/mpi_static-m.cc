...
int main(int argc, char **argv)
{
    ...
    memset(buf, 0, sizeof(int) * width * height * 2);

    // rank_id = size - 1 - rank_id;
    ...
    {
        int j = k * size + rank_id;
        MPI_Request req;
        // double y = j * unit_x + lower;
        ...
        for (int i = 0; i < width; i += 2)
        {
            ...
            in_set(x0, y0, buf + width * j + i);
        }
        // MPI_Ireduce(buf+k*size*width, image+k*size*width, size*width, MPI_INT, MPI_BOR, 0, comm, &req);
        MPI_Igather(buf + j * width, width, MPI_INT, image + k * size * width, width, MPI_INT, 0, comm, &req);
        reqs.push_back(req);
    }
    delete y0;
    delete x0;
    ...
    double sum_compute;
    MPI_Allreduce(&program_time, &sum_compute, 1, MPI_DOUBLE, MPI_SUM, comm);
    double avg_compute = sum_compute / size;
    double dif_compute = program_time - avg_compute;
    dif_compute = dif_compute * dif_compute;
    double var_compute;
    MPI_Allreduce(&dif_compute, &var_compute, 1, MPI_DOUBLE, MPI_SUM, comm);

    if (rank_id == 0)
        std::cout << program_time << " " << io_time << " " << comm_time << " " << var_compute;
